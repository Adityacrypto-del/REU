...
def make_ppt():
    ...
    try:
        ...
        # Clear out any existing placeholder slides in the template
        xml_slides = prs.slides._sldIdLst
        slides = list(xml_slides)
        for sld in slides:
            xml_slides.remove(sld)
            
        print("Cleared out original blank placeholder slides.")
    except Exception as e:
    ...
    for idx, slide_info in enumerate(slides_data):
        ...
        if slide_info['mermaid']:
            ...
            if generate_mermaid_image(slide_info['mermaid'], img_path):
                ...
                top = Inches(4.5) if slide_info['bullets'] else Inches(1.5)
                left = Inches(1.5)
                # Ensure the path exists
                if os.path.exists(img_path):
                    slide.shapes.add_picture(img_path, left, top, width=Inches(6.5))
            else:
    ...
    try:
        ...
        print(f"File {out_path} is open in PowerPoint. Saving to Final_Presentation_v2.pptx Instead.")
        prs.save(r"C:\Users\91948\OneDrive\Attachments\ReU\Final_Presentation_v2.pptx")
        print("Done.")
